		...
		try:
			for r in R:	
				...
				if '.png' in currImgPath:
					...
					msk = np.asarray(img_mask_gen(currImgPath))
					
					img_with_mask = np.zeros((224,224,4))
					
					img_with_mask[:,:,:-1] = img
					img_with_mask[:,:,-1] = msk
					
					input_images.append(img)
					output_images_with_masks.append(img_with_mask)
					mskb.append(np.reshape(msk, (224, 224, 1)))
		except:
			continue

		input_images = np.asarray(input_images)
		output_images_with_masks = np.asarray(output_images_with_masks)
		mskb = np.asarray(mskb)

		yield {'image_input': input_images}, {'sequential_2': output_images_with_masks}
...
def generate_data_trans_autoencoder(data_dict, batch_size):
	while 1:
		...
		while i < batch_size:
			...
			msk = np.asarray(img_mask_gen(out_img_path))
			out_data[:,:,3] = msk

			msk = np.reshape(msk, (224, 224, 1))
